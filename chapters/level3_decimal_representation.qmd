...
x^{(1)},\; x^{(2)},\; x^{(3)},\; \dots
$$
and, by our canonical rule, write each $x^{(n)}$ as
$$
x^{(n)} = 0.d_{n1} d_{n2} d_{n3}\ldots,
$$
...
**Acharya Bhaskar:** Now build a new decimal sequence $(e_k)_{k\ge1}$ by changing the $k$-th digit of the $k$-th listed number. For each $k$ define
$$
e_k :=
\begin{cases}
...
\end{cases}
$$
Thus $e_k\in\{0,\dots,9\}$ and, crucially, $e_k\neq d_{kk}$ for every $k$.

Let
$$
y := 0.e_1 e_2 e_3 \ldots .
$$
...
**Acharya Bhaskar:** For each $n$, the $n$-th digit of $y$ equals $e_n$, which by construction differs from $d_{nn}$, the $n$-th digit of $x^{(n)}$. Therefore $y\neq x^{(n)}$ for every $n$. In other words, $y$ is a point of $[0,1]$ not appearing in the supposed complete list. This contradicts the assumption that $[0,1]$ was countable.
